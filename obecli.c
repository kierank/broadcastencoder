...
static const char * output_opts[] = { "type", "target", "fec-columns", "fec-rows", "fec-type", "dup-delay", "arq-latency", "srt-password", "stream-id", "srt-type", NULL };
...
static int set_output( char *command, obecli_command_t *child )
{
    ...
    if( !strcasecmp( command, "opts" ) && str_len > tok_len )
    {
        ...
        char *srt_type = obe_get_option( output_opts[9], opts );
        ...
        if( srt_type ) {
            cli.output.outputs[output_id].srt_type = !strcmp(srt_type, "listener") ? 1 : 0;
        }
        obe_free_string_array( opts );
    }
...
static int start_encode( char *command, obecli_command_t *child )
{
    ...
    for( int i = 0; i < cli.num_output_streams; i++ )
    {
        ...
        if( input_stream && input_stream->stream_type == STREAM_TYPE_VIDEO )
        {
            ...
            /* FIXME support NTSC */
            if( cli.output_streams[i].downscale )
            {
                 cli.output_streams[i].avc_param.i_width = 720;
                 cli.output_streams[i].avc_param.i_height = 576;
            }
